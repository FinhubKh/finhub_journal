-- Extend get_my_journal_bundle breakdown with direction + outcome groups
-- so Overview BreakdownCard tabs (Session already existed; Direction/Outcome were empty).
...
begin
  ...
  by_direction as (
    select
      case lower(coalesce(nullif(trim(t.direction), ''), ''))
        when 'buy' then 'Long (Buy)'
        when 'long' then 'Long (Buy)'
        when 'sell' then 'Short (Sell)'
        when 'short' then 'Short (Sell)'
        else coalesce(nullif(trim(t.direction), ''), 'Other')
      end as name,
      count(*)::int as count,
      coalesce(sum(
        case
  ...
  by_outcome as (
    select
      case lower(coalesce(nullif(trim(t.result), ''), ''))
        when 'win' then 'Winning trades'
        when 'loss' then 'Losing trades'
        when 'be' then 'Break-even trades'
        else coalesce(nullif(trim(t.result), ''), 'Other')
      end as name,
      count(*)::int as count,
      coalesce(sum(
        case
    ...
    group by 1
    order by
      case lower(min(coalesce(nullif(trim(t.result), ''), '')))
        when 'win' then 1
        when 'loss' then 2
        when 'be' then 3
        else 4
  ...
  select jsonb_build_object(
    ...
    'breakdown', jsonb_build_object(
      ...
      'direction', coalesce((select jsonb_agg(to_jsonb(b)) from by_direction b), '[]'::jsonb),
      'outcome', coalesce((select jsonb_agg(to_jsonb(b)) from by_outcome b), '[]'::jsonb)
    ),
