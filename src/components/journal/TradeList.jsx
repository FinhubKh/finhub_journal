...
const filterField =
  'shrink-0 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-xs text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/15 disabled:cursor-not-allowed disabled:opacity-50';
...
export default function TradeList() {
  ...
  return (
    <div className={`${card} flex h-full min-h-0 w-full flex-col overflow-hidden`}>
      ...
      <div className="flex shrink-0 flex-nowrap items-center gap-2 overflow-x-auto border-b border-zinc-100 bg-zinc-50/50 px-4 py-3 md:px-5">
        ...
        />
        <input className={`${filterField} w-[132px]`} type="date" title="From" value={filters.from} onChange={(e) => setFilter('from', e.target.value)} />
        <input className={`${filterField} w-[132px]`} type="date" title="To" value={filters.to} onChange={(e) => setFilter('to', e.target.value)} />
        <button className={`${btnGhost} shrink-0`} type="button" onClick={clearFilters}>Clear</button>
      </div>
